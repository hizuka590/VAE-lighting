...
def cal_precision_recall_mae(pre, gt):
    ...
    for threshold in range(256):
        ...
        hard_prediction = torch.zeros(prediction.shape).to('cuda')
...
def cal_Jaccard(pre, gt):
    ...
    prediction = torch.zeros(pre.shape).to('cuda')
...
def cal_BER(pre, label, thr = 127.5):
    # print('==========ber========')
    # because shaodow part is far less than non shadow, so more weight to s_BER
    prediction = torch.zeros(pre.shape).to('cuda')
    label_tmp = label
    thr = thr / 255.
    # print(prediction)
    ...
    BER =  (0.5*shadow_BER + 0.5*non_shadow_BER)
